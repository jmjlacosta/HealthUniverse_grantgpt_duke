...
def get_bm25_corpus_index(corpus):
	corpus_path = os.path.join(f"dataset/{corpus}/bm25_corpus_{corpus}.json")
...
def get_medcpt_corpus_index(corpus):
	corpus_path = f"dataset/{corpus}/{corpus}_embeds.npy"
	nctids_path = f"dataset/{corpus}/{corpus}_nctids.json"
	...
	if os.path.exists(corpus_path):
		...
		model = AutoModel.from_pretrained("ncbi/MedCPT-Article-Encoder").to("cpu")
...
if __name__ == "__main__":
	...
	output_path = f"dataset/data/qid2nctids_results.json" #_{q_type}_{corpus}_k{k}_bm25wt{bm25_wt}_medcptwt{medcpt_wt}_N{N}.json"
